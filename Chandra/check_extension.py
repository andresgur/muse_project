...
parser.add_argument("-e", "--energy_range", help='Energy range considered for the simulation in keV', type=str, nargs=1,
                    default='0.3-10')
...
if e_range=='old':
    ...
    eranges = e_range.split("-")
    e_min = "%d" % (float(eranges[0]) * 1000)
    e_max = "%d" % (float(eranges[1]) * 1000)
    simdir = 'marx_outs'+'_'+e_range
    print("Using energy range %s" % e_range)
...
p = np.percentile(sblist, [50, 68, 100 - 68], axis=0)
...
if args.background is not None:
    ax_marx.axhline(y=bgbri[0], color='black', alpha=0.5, linestyle='--', linewidth=2, label='Background level')
