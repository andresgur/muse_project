...
def get_error(confpar):
    ...
    try:
        ...
        print(
            "Parameter uncertainties calculation must have failed for this parameter, returning np.inf"
        )
        return np.inf
    ...
    if np.isinf(negative_error) and np.isinf(positive_error):
        ...
        return (negative_error + positive_error) / 2.0
...
def create_line_model(
    ...
    max_FWHM_factor:int
        Sets the maximum allowed line broadening. This is determined as the initial FWHM value (the instrumental value) x factor
    FWHM_broad_factor:int
        Sets the maximum allowed line broadening for broad lines compared to the narrow component. Default is 8.
    ...
    for linename in lines.keys():
        ...
        velexpr = (
            f"((1 + {linename}_vel/ {ckms:.3f} ) / (1 - {linename}_vel/ {ckms:.3f})) **0.5 * (1 + {redshift:.8f}) * {line.wave:.4f}"
            if not "sky" in linename
            else f"((1 + {linename}_vel/ {ckms:.3f}) / (1 - {linename}_vel/ {ckms:.3f})) **0.5 * {line.wave:.4f}"
        )
        ...
        if "broad" in linename and (line.ref is None or line.ref not in lines.keys()):
            ...
            reflinename = refline.name + "_broad"
            # link velocity
            line_model.set_param_hint(
                "%s_vel" % linename, expr="%s_vel" % (reflinename),
            )
            line_model.set_param_hint("%s_center" % linename, expr=velexpr)
            # link sigma to the other line, and infer sigma for this line based on fwhm_vel
            line_model.set_param_hint(
                "%s_fwhm_vel" % linename, expr="%s_fwhm_vel" % reflinename
            )
            line_model.set_param_hint(
                "%s_sigma" % linename,
                expr="%s_fwhm_vel / 2.355 / %.3f * %s_center"
                % (linename, ckms, linename),
            )

            # link the flux
            line_model.set_param_hint(
                "%s_factor" % linename, value=line.th, min=line.low, max=line.up
            )
            # the amplitude is a factor X stronger than the narrow component
            line_model.set_param_hint(
                name="%s_amplitude" % linename,
                expr="%s_amplitude * %s_factor" % (refline, linename),
            )

        # new narrow line
        else:
            if line.ref is None or line.ref not in lines.keys():
                ...
                FWHMinst = (
                    get_instrument_FWHM(redshifted)[0] / redshifted * ckms
                )  # convert to velocity space
                minFWHM = FWHMinst * 0.5
                maxFWHM = (
                    FWHMinst * max_FWHM_factor
                    if not "sky" in linename
                    else FWHMinst * 2
                )
                FWHMvalue = (
                ...
                # link velocity
                line_model.set_param_hint(
                    "%s_vel" % linename, expr="%s_vel" % (refline.name),
                )
                line_model.set_param_hint("%s_center" % linename, expr=velexpr)
                # link sigma to the other line, and infer sigma for this line based on fwhm_vel
                line_model.set_param_hint(
                    "%s_fwhm_vel" % linename, expr="%s_fwhm_vel" % refline.name
                )
                line_model.set_param_hint(
                    "%s_sigma" % linename,
                    expr="%s_fwhm_vel / 2.355 / %.3f * %s_center"
...
def plot_fit(
    ...
    fig, axes = plt.subplots(
        2, 1, height_ratios=(2, 1), sharex=True, gridspec_kw={"hspace": 0.00}
    )
    ax = axes[0]
    if normalize:
    ...
    if lref:
        axes[-1].set_xlabel(rf"Velocity (km/s)")
    ...
    if initial_model is not None:
        ax.plot(
            xplot,
            get_initial_fit(initial_model, x) / norm,
            label=r"Initial Fit",
            ls="--",
        )
    for i, comp in enumerate(comps.keys()):
        if cont_prefix not in comp:
            ax.plot(
                xplotnew,
                (comps[comp] + comps[f"{cont_prefix}_"]) / norm,
                ls="--",
                zorder=-10,
                color=f"C{i+1}",
    ...
    axes[-1].set_ylabel(rf"Residuals")
    axes[0].legend()
    return fig, axes
...
def fit_spectrum(
    ...
    if np.all(spectrummask):
        ...
        if nparams > len(usefulfluxes):
            raise DofError(
                f"Insufficient degrees of freedom: {len(usefulfluxes)} data points but {nparams} parameters"
            )
    ...
    c1 = (usefulfluxes[-1] - usefulfluxes[0]) / (
        usefulwavelengths[-1] - usefulwavelengths[0]
    )
    ...
    for linename in fit_lines.keys():
        ...
        # no reference line
        if refline is None or refline not in fit_lines.keys():
            centroid = compute_shift(
                fit_lines[linename].wave,
                redshift,
                line_model.param_hints[f"{linename}_vel"]["value"],
            )[0]
            ...
            max_ind = (
                central_wavelength + 4
                if central_wavelength + 4 < len(usefulwavelengths)
    ...
    logger.debug(
        f"Fitting spectrum with {len(usefulfluxes)} datapoints with {nparams} parameters"
    )
    logger.debug("Initial model")
    logger.debug(line_model.param_hints)
    result = line_model.fit(
        usefulfluxes, x=usefulwavelengths, weights=1.0 / usefulstd, nan_policy="raise"
    )
    ...
    if result.redchi > 1000:
        ...
        # print(result.params)
        # print(np.isclose(best_fit, result.best_fit), np.isclose(result.data, usefulfluxes), np.isclose(result.weights, 1  / usefulstd))
        # spectrum.plot()
        # plt.plot(usefulwavelengths, best_fit, color="C1")
        # plt.show()
        return result, None
        raise ValueError(
            f"chi2 was massive in one fit ({result.redchi:.2f}). This likely suggest concurrent issues. Try lowering the number of cpus"
        )

    # if not np.isclose(chisq, result.chisqr):
    #   print("Warning: chisq from lmfit is not correct, using our own calculation", f"{result.redchi:.2f}", f"{(chisq / (len(usefulfluxes) - nparams)):.2f}")
    #  print("Warning: chisq from lmfit is not correct, using our own calculation", f"{chisq:.2f}", f"{result.chisqr:.2f}")
