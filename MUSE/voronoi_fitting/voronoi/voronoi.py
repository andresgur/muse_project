...
def _accretion(x, y, signal, noise, targetSN, pixelSize, quiet):
    ...
    for ind in range(1, n+1):
        ...
        if not quiet:
            print("{:6}/{}".format(ind, maxnum), end="\r")
            sys.stdout.flush()
...
def _reassign_bad_bins(classe, x, y, quiet):
    """
    ...
    #
    bad = classe == 0
    # Added a memory safety net for very large arrays (i.e. MUSE)
    #
    if xnode.size * x[bad].size > 1e6:
        badSize = x[bad].size
        index = np.zeros(badSize, dtype='int')
        for i, badIndex in enumerate(np.where(classe == 0)[0]):
            if not quiet:
...
#----------------------------------------------------------------------------
# multiprocessing function must be picklable (i.e. top-level)
def classe_argmin(fargs):
    i, x, y, xnode, ynode, scale = fargs
    return np.argmin(((x[i] - xnode)**2 + (y[i] - ynode)**2)/scale**2)

def _cvt_equal_mass(x, y, signal, noise, xnode, ynode, quiet, wvt, n_cpu):
    """
    ...
    xSize = x.size
    xNodeSize = xnode.size
    for it in range(1, xNodeSize):  # Do at most xnode.size iterations
        ...
        #
        # Added a memory safety net for very large arrays (i.e. MUSE)
        # Also multiprocessed since this is most time-consuming part
        if xNodeSize * xSize > 1e6:
            classe = np.empty(xSize, dtype='int')
            p = Pool(n_cpu)
            classe = np.array(p.map(classe_argmin, zip(range(xSize),
                                                   repeat(x),
                                                   repeat(y),
                                                   repeat(xnode),
                                                   repeat(ynode),
        ...
        if not quiet:
            print('Iter: %4i  Diff: %.4g' % (it, diff), end="\r")
...
def _compute_useful_bin_quantities(x, y, signal, noise, xnode, ynode, scale, n_cpu):
    """
...
def voronoi_2d_binning(x, y, signal, noise, targetSN, cvt=True,
                         pixelsize=None, plot=True, quiet=True, wvt=True, n_cpu=2):
    """
    PURPOSE:
          Perform adaptive spatial binning of Integral-Field Spectroscopic
          (IFS) data to reach a chosen constant signal-to-noise ratio per bin.
    ...
    xNode, yNode = _reassign_bad_bins(classe, x, y, quiet)
    print(xNode.size, ' good bins.')
    if cvt:
        print('Modified Lloyd algorithm...')
        xNode, yNode, scale, it = _cvt_equal_mass(x, y, signal, noise, xNode, yNode, quiet, wvt, n_cpu)
        print(it, ' iterations.')
    else:
        scale = 1.
    classe, xBar, yBar, sn, area = _compute_useful_bin_quantities(x, y, signal, noise, xNode, yNode, scale, n_cpu)
    w = area == 1
