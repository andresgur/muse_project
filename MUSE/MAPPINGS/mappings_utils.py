...
"""
Pne2014 and Allen models to be updated (missing some informations in the csvs but the database is down for now so can't check what's missing)
"""
...
def shock_model(modelname):
    ...
    ----------
    model: str
        Gives which type of model is used here
    ...
    modelfiles=glob.glob(modelname+"*.csv")
    ...
    for i in range(len(modelfiles)):
        ...
        if 'Allen' in modelname:
            abund_list[i]=filename[filename.find('_')+1:filename.find('.csv')]
        
        elif 'Gutkin' in modelname:
            abund_strlist[i]=filename[filename.find('d')+1:filename.find('C')-1]
            
        elif '3MdB' or 'PNe' in modelname:
    ...
    class shock_model_single:
       ...
       """
        ...
        def grid(self, dens,mag_fld):
            return self.csv[self.csv.dens==dens][self.csv.mag_fld==mag_fld][['shck_vel','OIII_Hb','NII_Ha','SII_Ha','OI_Ha']]
    
        def lgrid(self, dens,mag_fld):
            #we keep the velocity in linear and check for the presnece of infs. to transform into nans.
            log_csv=self.csv[self.csv.dens==dens][self.csv.mag_fld==mag_fld][['shck_vel','OIII_Hb','NII_Ha','SII_Ha','OI_Ha']]
            log_csv=log_csv.copy()
            log_csv=log_csv.replace([np.inf,-np.inf],np.nan)
            log_csv[log_csv.columns[1:]]=np.log10(log_csv[log_csv.columns[1:]])
            return log_csv
            
        #two dimensionnal surfaces grids functions
        def d_grid(self,dens):
            return self.csv[self.csv.dens==dens][['mag_fld','shck_vel','OIII_Hb','NII_Ha','SII_Ha','OI_Ha']],

        def d_lgrid(self,dens):
            #we keep the velocity in linear and check for the presnece of infs. to transform into nans.
            log_csv=self.csv[self.csv.dens==dens][['mag_fld','shck_vel','OIII_Hb','NII_Ha','SII_Ha','OI_Ha']]
            log_csv=log_csv.copy()
            log_csv=log_csv.replace([np.inf,-np.inf],np.nan)
            log_csv[log_csv.columns[2:]]=np.log10(log_csv[log_csv.columns[2:]])
            return log_csv
    ...
    class shock_mod:
        ...
        def abund_single(self,abundance):
            ...
            class shock_model_with_abundances:
                def __init__(self):
                    #dichotomy since Gutkin models will have 
                    if type(abundance)==float or np.float64:
                    ...
                    elif type(abundance)==str or type(np.array(abundance)[0]):
